...
We have defined a new class \code{vtkPolyDataIntersection} that computes the surface intersection between two surface meshes and can optionally split the input meshes by the surface intersection. The surface meshes must consist of triangles. Two options are available for this filter:
\begin{verbatim}
...
There are two ways to compute the surface mesh resulting from a boolean operation on two surface meshes: setting up a VTK pipeline or using a new class \code{vtkPolyDataBooleanOperationFilter}.

\subsection{Setting up a VTK Pipeline for Boolean Operations on Surfaces}

To compute a surface resulting from a boolean operation on surface meshes, the pipeline of classes shown in Figure \ref{fig:Pipeline} can be used to compute the desired surface. The two instances of \code{vtkThreshold} are required to select different portions from each of the input geometries.
...
\code{vtkAppendFilter}

$\downarrow$

\code{vtkDataSetSurfaceFilter}
\end{center}
\itkcaption{Possible pipeline to be used to compute results of boolean operations on surface meshes.}
\label{fig:Pipeline}
\end{figure}

\subsection{vtkPolyDataBooleanOperationFilter}

For convenience, we have included a single class that replicates the functionality  class for computing boolean operations on surface meshes, \code{vtkPolyDataBooleanOperationFilter}, that encapsulates the functionality of the pipeline above. This class has three options. The desired boolean operation is specified with one of
\begin{verbatim}
