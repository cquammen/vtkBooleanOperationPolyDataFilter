...
We have defined a new class \code{vtkPolyDataIntersection} that encapsulates the algorithm described above to compute the surface intersection between two surface meshes and optionally split the input meshes by the surface intersection. The surface meshes must consist of triangles. Two options are available for this filter:
\begin{verbatim}
...
There are two ways to use the classes we have developed to compute the surface mesh resulting from a boolean operation on two surface meshes: setting up a VTK pipeline or using a new class \code{vtkPolyDataBooleanOperationFilter}.

\subsection{Setting up a VTK Pipeline for Boolean Operations on Surfaces}

To compute a surface resulting from a boolean operation on surface meshes, the pipeline of classes shown in Figure \ref{fig:Pipeline} can be used to compute the desired surface. The two arrows between \code{vtkPolyDataIntersection} and \code{vtkPolyDataDistance} indicate that the two output surfaces from \code{vtkPolyDataIntersection} are set as the input to \code{vtkPolyDataDistance}. The two instances of \code{vtkThreshold} operate on different outputs of \code{vtkPolyDataDistance}.
