...
To compute a surface resulting from a boolean operation on surface meshes, the pipeline of classes shown in Figure \ref{fig:Pipeline} can be used to compute the desired surface. The two arrows between \code{vtkPolyDataIntersection} and \code{vtkPolyDataDistance} indicate that the two output surfaces from \code{vtkPolyDataIntersection} are set as the input to \code{vtkPolyDataDistance}. The two instances of \code{vtkThreshold} operate on the two outputs of \code{vtkPolyDataDistance}. The example \verb+PipelineExample.cxx+ included with the source code in this article shows how to set up a pipeline to compute boolean operations.
