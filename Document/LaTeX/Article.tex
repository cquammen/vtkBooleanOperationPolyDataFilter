...
To compute a surface resulting from a boolean operation on surface meshes, the pipeline of classes shown in Figure \ref{fig:Pipeline} can be used to compute the desired surface. The two instances of \code{vtkThreshold} and two instances of \code{vtkDataSetSurfaceFilter} are required to select different portions from each of the input geometries.
...
\code{vtkImplicitPolyData} $\rightarrow$ \code{vtkClipPolyData} \hspace{1em} \code{vtkClipPolyData} $\leftarrow$ \code {vtkImplicityPolyData}
